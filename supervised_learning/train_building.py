...
from supervised_learning.dataset import CustomSoundDataset, OutPut_DataSet
...
def input_transform(data1,data2):
  ...
  random_ = np.random.random(1)
  input_data = np.concatenate([input_data,random_])
...
if __name__ == '__main__':
  ...
  output_dataset = OutPut_DataSet(capacity, batch_size, query_shape = [5,6], answer_shape = [20,], transform=None)
  ...
  for t in range(epochs):
    ...
    train_loop(train_dataloader, model, loss_fn, optimizer, writer, t, device,output_dataset)
